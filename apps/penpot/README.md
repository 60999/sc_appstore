...
**正确示例**：
- 使用域名（推荐）：`https://penpot.example.com`
- 使用公网 IP：`http://123.45.67.89:9001`
- 使用 1Panel 反向代理：`https://penpot.example.com`

**错误示例**：
- ❌ `http://localhost:9001` - 用户无法访问
- ❌ `http://127.0.0.1:9001` - 用户无法访问
- ❌ `http://172.16.0.1:9001` - 内网地址用户无法访问

**为什么必须填写正确的访问地址？**
- Penpot 使用此地址生成设计分享链接、邀请链接
- 前端资源加载依赖正确的 URL
- 设计协作功能需要正确的域名配置
